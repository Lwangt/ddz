...
以上步骤全部自动执行，**严禁询问用户确认**。即：
- `git add -A && git commit -m "..."` 直接执行
- `git push origin main` 直接执行
- `node deploy.js` 直接执行
- 三步连做，中间不停顿、不提问

### 部署铁律
- **只部署到 8081 端口**，绝不触碰其他端口
- **部署完成后必须测试** `http://47.112.7.97:8081/ddz/` 能正常访问斗地主（检查返回 `lang="zh-CN"`）
- 不影响服务器上其他任何服务
